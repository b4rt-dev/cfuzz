...
u_char *PrbTemplate(u_char *dstAddress, int *packetSize, u_char * radioTapHeader, u_char *myMAC, int step)
{

    #define PRBtemplateIES  (3)   //number of information elements
    ...
    infoElem taggedParams[PRBtemplateIES] = {ssid, suppRates, ds};

    //length of all info elements, including id and len field
    int len_taggedParams = 0;
    for(int i = 0; i < PRBtemplateIES; i++)
    {
